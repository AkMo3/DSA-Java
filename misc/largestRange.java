...
public class largestRange {
  ...
  public static int longestRange(int[] nums) {
    int longestRange = 0;
    HashMap<Integer, Boolean> num = new HashMap<>(); // Stores a mapping of a number to whether the current number is
                                                     // part of a particular consecutive range or not
    for (int x : nums)
      num.put(x, true);
    for (int x : nums) {
      if (!num.get(x))
      ...
      if (currentRange > longestRange) // Store longest range at every interation
  ...
  public static void main(String[] args) {
    ...
    // The longest consecutive number range is of length 1 i.e. any of the element
    // alone
    assert longestRange(new int[] { 0, 1, 2, 3, 4, 7, 6, 5 }) == 8;
    // The longest consecutive number range is of length 8 i.e. {0, 1, 2, 3, 4, 5,
    // 6, 7}
  }
}
